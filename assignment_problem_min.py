...
def hungarianMethod(matrix):
    ...
    deleted_row = []
    marked = []
    with open("data.jnfljc", "a") as file:
        file.write("Step 1: Row reduction and Column Reduction.\n")


    #print("step1: row reduction and col reduction")
    matrix = row_reduction(matrix)
    matrix = col_reduction(matrix)

    with open("data.jnfljc", "a") as file:
        file.write("\n")
        file.write("step2: row scanning (col scanning)\n")
        file.write("\n")
        file.write("row scanning: if the row has exactly one zero, mark the zero and draw a vertical line")

   # print()
   # print("step2: row scanning (col scanning)")
   # print()
   #print("row scanning: if the row has exactly one zero, mark the zero and draw a vertical line")
...
def row_reduction(matrix):
    ...
    with open("data.jnfljc", "a") as file:
        file.write("\n")
        file.write("done row reduction\n")
        file.write(str(matrix) + "\n")
    #print()
    #print("done row reduction")
    #print(matrix)
    return matrix
...
def adjust_matrix(matrix, deleted_row, deleted_col):
    ...
    with open("data.jnfljc", "a") as file:
        file.write("\n")
        file.write("done adjusting the matrix\n")
        file.write(str(matrix) + "\n")

    #print("done adjusting matrix")
    #print(matrix)
    #print()
    return matrix
